...
const longMessage = require('./longMessage.js')
...
class cbusAdmin extends EventEmitter {
  ...
  preOpcodeProcessing(cbusMsg) {
    ...
    try {
      ...
      winston.error({
        message: name + `: preOpcodeProcessing
        ${err} 
  ...
  postOpcodeProcessing(cbusMsg) {
    ...
    try {
      ...
      winston.error({
        message: name + `: postOpcodeProcessing
        ${err} 
        ${JSON.stringify(cbusMsg)}
        `
      })
    ...
    try {
      if (this.nodeConfig.nodes[nodeNumber]) {
        ...
        if ((moduleIdentifier != undefined) && (moduleVersion != undefined) && (processorType != undefined)) {
          winston.debug({
            message: name + `: getNodeDescriptor:
            nodeNumber ${nodeNumber}
          ...
          if (moduleDescriptor) {
            ...
            winston.info({
              message: name + `: getNodeDescriptor: failed to load file
              nodeNumber ${nodeNumber}
              moduleIdentifier ${moduleIdentifier}
              moduleVersion ${moduleVersion}
              processorType ${processorType}`
          ...
          winston.debug({
            message: name + `: getNodeDescriptor: missing information
            nodeNumber ${nodeNumber}
            moduleIdentifier ${moduleIdentifier}
            moduleVersion ${moduleVersion}
            processorType ${processorType}`
